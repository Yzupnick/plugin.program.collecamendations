...
def addMovieDirectory(movie_name, movie_id, menu_number, thumbnail_path):
    ...
    CP_ADD_VIA_TITLE = 'XBMC.RunPlugin(plugin://plugin.video.couchpotato_manager/movies/add?title=%s)'
    TRAKT_ADD_URL = 'XBMC.RunPlugin(plugin://plugin.video.trakt_list_manager/movies/add?title=%s)'
    list_item.addContextMenuItems([('Add Movie to CouchPotato', CP_ADD_VIA_TITLE % movie_name),('Add Movie on Trakt.tv', TRAKT_ADD_URL % movie_name)])
